...
export default class PathLogin extends React.Component {
  ...
  rlogin=(e)=>{
    ...
    ListPathalogyService.login(loginUser).then((res) => {  
      let patient = res.data;  
      const ddata=JSON.stringify(patient);
      const jsondata=JSON.parse(ddata);
      console.log(jsondata);
      localStorage.setItem("pathemail",jsondata[0].email_id);
      console.log(jsondata[0].emailId);
      localStorage.setItem("pathid",jsondata[0].pathalogy_id);
      console.log(jsondata[0].pathalogyid);
      window.location.href="/pathalogy/getAllPatientPathalogy";
    ...
    return (
      ...
      <div className="col-4 p-5 shadow-lg bg-light">
          {/* on submitting the form signinUserHandler function will be called */}
           <h1 className='text-center pb-4 text-warning'> Pathology Login </h1>
